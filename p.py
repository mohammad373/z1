...
'adm/index.brf','adm.brf','affiliate.brf','adm_auth.brf','memberadmin.brf','administratorlogin.brf','cpanel','cpanel.php','cpanel.html']
    target = input(Fore.BLUE + "[!] ~ " + Fore.GREEN + "Pleass Enter Tour Domain Target" + Fore.BLUE + " ==>  ")
    if target == "" or None:
        try:
    ...
    if r1.status_code == 200:
        print(Fore.GREEN + "[+] ~ Your Target Is Found ;)")
    if r1.status_code != 200:
        try:
            ...
            sys.exit()
    target2 = input(Fore.BLUE + "[!] ~ " + Fore.GREEN + "Are You Add The Plugin ? (Y/n)" + Fore.BLUE + " ==>  ")
    if target2 == "" or None:
        try:
            print(Fore.RED + "[-] ~ Error : Your Input Is None ;(")
    if target2.lower() not "y" or target2.lower() not "n":
        try:
    ...
    if target2.lower() == "n":
        None
    if target2.lower() == "y":
        print(Fore.GREEN + "\n[+] ~ If Your Input Is Exit Else Input Is Break ;)")
        time.sleep(1)
        while True:
            i = input(Fore.BLUE + "[!] ~ " + Fore.GREEN + "Pleass Enter Plugins" + Fore.BLUE + " ==> ")
            if i == "" or None or i.lower() == "exit":
                break
            else:
                my_list.append(i)
    ...
    if w1 == "1":
        for m in target3:
            target4 = target + "/" + m
            target5 = requests.get(target4)
            if target5.status_code == 200:
                print(Fore.GREEN + target4)
            if target5.status_code != 200:
                print(Fore.RED + target4)
    if w1 == "2":
        for nm in target3:
            target6 = target + "/" + nm
            target7 = requests.get(target6)
